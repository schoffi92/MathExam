...
public partial class FamilyGameViewModel : ObservableObject
{
    ...
    public string TurnText => $"{_game.Current.Name}'s turn";
    public string RoundText => $"Round {_game.Round}";
    public string ElapsedText => $"Elapsed: {Session.Elapsed.ToString(@"hh\:mm\:ss")}";
    ...
    public string ButtonText => IsAnswered ? "Next player" : "Send";
    ...
    public IReadOnlyList<ScoreRow> Scoreboard => _game.Players
        .Select(p => new ScoreRow(
            p.Name,
            $"{p.Session.CorrectCount} / {p.Session.AnsweredCount} correct",
            $"Level {p.Session.Level}",
            p == _game.Current))
        .ToList();
