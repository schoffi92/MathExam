...
| Type | Responsibility |
|---|---|
| `Operation` | `Add`, `Subtract`, `Multiply`, `Divide`, `Power`, `Root`, `Convert`. `Symbol()` returns `+ − × ÷ xⁿ √ ↔`. `IsBasic()` is true for + − × ÷. |
...
| `HelpViewModel` | The "How to use" screen: `SectionCount` sections from `Help_S{n}_Title` / `Help_S{n}_Body`. `FillLabels` replaces each `«Key»` with that key's text, so the help names buttons exactly as the screens do. Its language list binds to the shared `DisplayViewModel.Language`; on a change it reloads `Sections` (the window rebuilds the rest of the screen), and it stops listening on Close. Implements `IScrollsItself`. |
...
| `SummaryViewModel` | Read-only snapshot of the finished session, plus the level range, any history save error, for timed games "Time's up" and the personal best or a new one, and for tests the score and the review of `Mistakes`. |
| `HistoryViewModel` | Two tabs. Games: `HistoryRow`s (newest first) with a totals and a per-operation line, filtered by a `HistoryFilter` (all players or one). Players: `PlayerOverviewRow`s from `PlayerStats.Overview`. `ExportCommand` saves the shown records as CSV. |
...
`WorksheetPdf.Write(stream, content, regular, bold)` draws an A4 PDF: one task page per `WorksheetSheet` (a version with its label), then one answer page per sheet, with SkiaSharp's `SKDocument.CreatePdf` (SkiaSharp comes with Avalonia, so there is no extra dependency). `InterFonts` loads the bundled Inter font from `avares://Avalonia.Fonts.Inter/Assets/` for it, so every language and the maths symbols render. Task pages list up to 30 tasks in two columns (at least ten rows), drawn from `MathTask.ToParts()`: text runs, a line for a missing number, a box for a missing operator, a small raised box for a missing exponent or degree, and superscript digits from `MathTask.ToSuperscript`. Long equations are scaled to fit their column. Answer pages use `ToParts(revealAnswer: true)`.
...
- profiles (storage, name validation), daily goal counts, per-profile levels and bests, the player overview, game modes, tests and their mistakes
...
- unit conversions: exact within one quantity, factors, both directions, decimals, display and explanations
