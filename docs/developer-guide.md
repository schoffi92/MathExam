...
| `MainViewModel` | Owns navigation, the `HistoryStore` and the shared `DisplayViewModel` (also exposed to the menu as `MenuViewModel.Display`). Reuses one `MenuViewModel`, so settings persist between games. Creates the `DifficultyAdjuster` at the resume level, starts timed challenges (`ChallengeTime`, 1 minute), looks up the personal best before saving each finished game that has answers, and writes worksheets and CSV exports through `IFileSaver`, which `MainWindow` implements with the system save dialog. |
| `MenuViewModel` | Min/max are bound as strings, so invalid input can be reported instead of silently rejected. `ErrorMessage` and the commands' `CanExecute` reuse `GameSettings.Validate()`. `Numbers` is a `NumberOption` from a combo box; `UseMissingOperator`, `UseTimed`; `WorksheetCommand` shows the save result in `StatusText`. The view has two columns (settings, display). |
| `GameViewModel` | Solo game. `Equation` is the current `MathTask`, drawn by `Views/EquationBlock`, a `TextBlock` that turns `ToParts()` into runs: superscript digits use the font's Unicode superscripts, and a hidden `?` exponent is a smaller run with `BaselineAlignment.Superscript`. `State` (`Answering`/`Correct`/`Wrong`) drives the button text and read-only state. `IsCorrect`/`IsWrong` toggle the views' `correct`/`wrong` style classes. A single `SubmitOrNextCommand` handles both steps; `Answers.Submit` reads the text as an operator or a `Rational` as the task needs. `PickOperatorCommand` backs the + − × ÷ buttons of missing-operator tasks (`ShowOperatorButtons`). `ExplanationText` shows `Explainer`'s working after a wrong answer. An Avalonia `DispatcherTimer` refreshes `ClockText` (elapsed, or time left) and stops a timed game when `IsTimeUp`. |
| `SummaryViewModel` | Read-only snapshot of the finished session, plus the level range, any history save error, and for timed games "Time's up" and the personal best or a new one. |
| `HistoryViewModel` | Formats records into `HistoryRow`s (newest first) for a read-only `DataGrid`, and adds a totals line and a per-operation accuracy line from `HistoryTotals`. `ExportCommand` saves the CSV. |
| `FamilySetupViewModel` | 2–4 `PlayerNameEntry` rows (add/remove), validated with `FamilyGame.ValidatePlayers`, and `TasksPerTurn` (a `decimal?`, as `NumericUpDown` binds to, clamped to 1–10 on start). Reused, so names are kept; `Open(settings)` receives the menu settings each time. |
...
`WorksheetPdf.Write(stream, content, regular, bold)` draws an A4 PDF with SkiaSharp's `SKDocument.CreatePdf` (SkiaSharp comes with Avalonia, so there is no extra dependency). `InterFonts` loads the bundled Inter font from `avares://Avalonia.Fonts.Inter/Assets/` for it, so every language and the maths symbols render. Page 1 lists 20 tasks in two columns, drawn from `MathTask.ToParts()`: text runs, a line for a missing number, a box for a missing operator, a small raised box for a missing exponent or degree, and superscript digits from `MathTask.ToSuperscript`. Long equations are scaled to fit their column. Page 2 is the answer key (`ToParts(revealAnswer: true)`).
...
- **Strings:** every UI text lives in `Resources/Strings.resx` (English, the fallback) with one `Strings.<lang>.resx` per translation (`fr`, `de`, `hu`, `it`, `es`, `pl`, `cs`, `fi`, `pt`, `sv`, `nb`, `hr`, `sl`, `sk`, `tr`, `el`, `ro`, `nl`). Norwegian is `nb` (Bokmål), because that is where `nb-NO` systems fall back to. `MathExam.Core` has its own small set for the validation messages. The build generates a `Strings` class from each neutral file (`StronglyTyped*` metadata in the `.csproj`), so keys are checked at compile time. The app's class is public so views can use `{x:Static res:Strings.Key}`.
...
- `Explainer`: every strategy, including negative numbers, powers, roots and fractions
- timed sessions with a manual clock, per-operation counts, personal best, the new record fields surviving a save, per-operation totals, and CSV output (format, quoting, older records)
